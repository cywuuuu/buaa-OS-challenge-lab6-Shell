...
/*** exercise 5.5 ***/
u_int
diskaddr(u_int blockno) // diskaddr 函数用来计算指定磁盘块对应的虚存地址
// 磁盘与内存：
//建立起了磁盘地址空间和进程虚存地址空间之间的缓存映射
{
	if (super && blockno >= super->s_nblocks) {
		user_panic("fs/fs.c: reading non-existent block %08x\n", blockno);
	}
...
unmap_block(u_int blockno)
{
    ...
    // can't be unmap directly.	若是脏的要进行写回操作, writeblock之后，在free掉 	
		if (!block_is_free(blockno) && block_is_dirty(blockno)) {
			write_block(blockno); 
		}
		// 取消serve进程 va处的映射关系， 相当于盘块在内存中消失
		r = syscall_mem_unmap(0, va); 
		if (r < 0) user_panic("fs/fs.c: unmap failed"); 
...
//	
// isnew != 0时检查是不是在内存中， isnew => 0 block不是新的已经在内存，
//  isnew => 1 block 是新ide_read到内存里的
...
read_block(u_int blockno, void **blk, u_int *isnew)
{
	...
	//	writef("begin to read %dth block\n", blockno);//
...
free_block(u_int blockno)
{
	...
	if (blockno >= super->s_nblocks || blockno == 0 || super == 0) {
...
dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	// writef("nblock is %d, fsize is %d\n", nblock, dir->f_size); 
	// 通过文件控制块 File dir我们知道
	//这个目录会有nblock个块来存储 
	for (i = 0; i < nblock; i++) {
		// Step 2: Read the i'th block of the dir.
		// Hint: Use file_get_block.
		// 可以通过dir文件控制块，得到存储目录文件的第i块blk地址
		...
		// 目录文件的每一块都存着FILE2BLK个文件控制块
		
		for (j = 0; j < FILE2BLK; j++) {
			f = (struct File*)blk + j; 
			//writef("%s has %s\n", (char *)(dir->f_name), (char *)(f->f_name)); 
...
{
//walk_path(char *path, struct File **pdir, struct File **pfile, char *lastelem)
//            给出path    返回所在目录File          文件        最后一个可以找的path
