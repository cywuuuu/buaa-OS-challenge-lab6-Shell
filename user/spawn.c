...
init_stack(u_int child, char **argv, u_int *init_esp)
{
	...
	for (argc=0; argv[argc]; argc++)
		tot += strlen(argv[argc])+1;//1就是'\0'
	// argv是参数指针，指向stringstring
	// Make sure everything will fit in the initial stack page
	if (ROUND(tot, 4)+4*(argc+3) > BY2PG)
		return -E_NO_MEM;

	// Determine where to place the strings and the args array
	// TOP - tot 就是string start
...
/*int spawn(char *prog, char **argv)
{
	...
	if (r < 0) user_panic(" read elfhdr failed\n"); 
	ehdr = elfbuf; 

	fsize = ((struct Filefd *) num2fd(fd))->f_file.f_size;

	// Step 2: Allocate an env (Hint: using syscall_env_alloc())
	if (ehdr->e_type != 2 || !usr_is_elf_format(ehdr)|| fsize < 4) user_panic("file not executeble\n"); 
	if (child_envid = syscall_env_alloc() < 0) user_panic("child env alloc failed\n"); 
	...
	for (i = 0; i < ehdr->e_phnum; i++) {
		if ((r = seek(fd, (ehdr->e_phoff + i*ehdr->e_phentsize)))) user_panic("seek failed\n"); 
		if ((r = readn(fd, elfbuf, ehdr->e_phentsize))) user_panic("read Program header table entry failed\n"); 
		//获取段头项
		phdr = elfbuf; 
		if (phdr->p_type == PT_LOAD) {
			if (r = usr_load_elf(fd, phdr, child_envid) < 0) user_panic("loading Program failed\n"); 
...
int spawn(char *prog, char **argv)
{
    ...
    //writef("开始spawn %s\n", prog);
	if((r=open(prog, O_RDONLY))<0){
		writef("spawn ::open program not found !\n");
		return r;
	}
	// Your code begins here
	// Before Step 2 , You had better check the "target" spawned is a execute bin 
